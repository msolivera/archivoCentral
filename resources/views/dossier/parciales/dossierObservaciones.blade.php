    ...
    <div class="card" style="background-color: #E6EFF6;">
        <div class="card-body">
            ...
            <table id="observacionTable" class="table table-bordered table-striped table-sm">
                ...
                <tbody>
                    @foreach ($dossierObservaciones as $dossierObservacion)
                        <tr>
                            <td>{{ $dossierObservacion->id }}</td>
                            <td>{{ $dossierObservacion->observacion }}</td>
                            <td>
                                <form method="POST"
                                    action="{{ route('dossierObservaciones.destroy', $dossier->id) }}"
...
<div class="modal-dialog" role="document">
    <form method="POST" action="{{ route('dossierObservaciones.store', $dossier->id) }}">
        {{ csrf_field() }}
